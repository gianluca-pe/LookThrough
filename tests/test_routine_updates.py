...
def test_session_deduplicates_prices_and_lists_current_source_rows(
    app: Flask,
) -> None:
    records = _records(app)
    with app.app_context():
        portfolio = db.session.get(Portfolio, records["portfolio_id"])
        session = build_routine_update_session(portfolio, date(2026, 8, 30))

        prices = {row["instrument_id"]: row for row in session["price_rows"]}
        assert set(prices) == {records["fund_id"], records["bond_id"]}
        assert prices[records["fund_id"]]["account_names"] == (
            "Bank · Investment EUR",
            "Bank · Investment USD",
        )
        assert prices[records["fund_id"]]["status"] == "current"
        statements = {
            row["registration_id"]: row for row in session["statement_rows"]
        }
        assert set(statements) == {
            records["wrapper_registration_id"],
            records["deposit_registration_id"],
        }
        assert statements[records["deposit_registration_id"]]["status"] == "current"
        assert {row["currency_code"] for row in session["cash_rows"]} == {
            "EUR", "USD"
        }
        assert [
...
def test_routine_rows_follow_institution_then_account_work_order(
    app: Flask,
) -> None:
    records = _records(app)
    with app.app_context():
        portfolio = db.session.get(Portfolio, records["portfolio_id"])
        alpha_institution = Institution(portfolio=portfolio, name="A Bank")
        zulu_institution = Institution(portfolio=portfolio, name="Z Bank")
        db.session.add_all([alpha_institution, zulu_institution])
        db.session.flush()
        alpha_account = Account(
            portfolio_id=portfolio.id,
            institution_id=alpha_institution.id,
            name="Zulu account",
            account_type="brokerage",
            default_currency_code="EUR",
            is_multicurrency=False,
        ...
        zulu_account = Account(
            portfolio_id=portfolio.id,
            institution_id=zulu_institution.id,
            name="Alpha account",
            account_type="brokerage",
            default_currency_code="EUR",
            is_multicurrency=False,
        ...
        alpha_price = Instrument(
            portfolio_id=portfolio.id,
            name="Zulu price instrument",
            instrument_type="stock",
            valuation_currency_code="EUR",
            is_active=True,
        )
        zulu_price = Instrument(
            portfolio_id=portfolio.id,
            name="Alpha price instrument",
            instrument_type="stock",
            valuation_currency_code="EUR",
            is_active=True,
        )
        alpha_statement = Instrument(
            portfolio_id=portfolio.id,
            name="Zulu statement",
            instrument_type="other",
            valuation_currency_code="EUR",
        ...
        zulu_statement = Instrument(
            portfolio_id=portfolio.id,
            name="Alpha statement",
            instrument_type="other",
            valuation_currency_code="EUR",
            is_active=True,
        )
        ...
        registrations = [
            PositionRegistration(
                account_id=alpha_account.id,
                instrument_id=alpha_price.id,
                tracking_mode="transaction_tracked",
                opening_date=date(2026, 8, 1),
            ),
            PositionRegistration(
                account_id=zulu_account.id,
                instrument_id=zulu_price.id,
                tracking_mode="transaction_tracked",
                opening_date=date(2026, 8, 1),
            ),
            PositionRegistration(
                account_id=alpha_account.id,
                instrument_id=alpha_statement.id,
                tracking_mode="statement_valued",
                opening_date=date(2026, 8, 1),
            ),
            PositionRegistration(
                account_id=zulu_account.id,
                instrument_id=zulu_statement.id,
                tracking_mode="statement_valued",
                opening_date=date(2026, 8, 1),
            ),
        ]
        ...
        db.session.add_all(registrations + [transaction])
        db.session.flush()
        db.session.add_all(
            [
                Posting(
                ...
                ValuationObservation(
                    position_registration_id=registrations[2].id,
                    effective_date=date(2026, 8, 20),
        ...
        session = build_routine_update_session(portfolio, date(2026, 8, 30))
        cash_order = [row["institution_name"] for row in session["cash_rows"]]
        statement_order = [
            row["institution_name"] for row in session["statement_rows"]
        ]
        price_order = [row["account_names"][0] for row in session["price_rows"]]
        assert cash_order == sorted(cash_order)
        assert statement_order == sorted(statement_order)
        assert price_order == sorted(price_order)


def test_values_page_makes_routine_session_primary_and_keeps_individual_forms(
    client: FlaskClient, app: Flask
) -> None:
    records = _records(app)
    body = client.get("/values?as_of=2026-08-30").get_data(as_text=True)

    assert body.index("Routine update") < body.index("Individual updates")
    assert 'action="/values/routine/cash"' in body
    assert 'action="/values/routine/statements"' in body
    assert 'action="/values/routine/prices"' in body
    assert 'action="/values/routine/fx"' in body
    assert f'name="price_{records["fund_id"]}"' in body
    assert body.count(f'name="price_{records["fund_id"]}"') == 1
    assert f'name="statement_{records["deposit_registration_id"]}"' in body
    assert "FD USD 1" in body and "Fixed deposit" in body
    assert "1 USD = RATE EUR" in body
    assert 'action="/values/price"' in body  # targeted exception path remains
    assert '<details class="card">' in body
    targeted = client.get(
        "/values",
        query_string={"as_of": "2026-08-30", "instrument": records["fund_id"]},
    ).get_data(as_text=True)
    assert '<details class="card" open>' in targeted
    scripts = [re.sub(r"\?v=\d+", "", tag) for tag in re.findall(r"<script[^>]*>", body)]
    assert scripts == ['<script src="/static/js/enhance.js" defer>']
    # The routine card introduces the task; the setup-pathway intro copy must
    # not duplicate above it on the maintenance route.
    assert "Enter what you know" not in body
    # The shared date is explicit about what it drives.
    assert "Reloads every section below" in body
    # All four source tables use the standard data-table presentation and
    # the date row stays compact.
    assert body.count('class="data routine-table"') == 4
    assert 'class="routine-date-form"' in body
    setup_body = client.get("/setup/values").get_data(as_text=True)
    assert "Enter what you know" in setup_body  # guided pathway keeps it
